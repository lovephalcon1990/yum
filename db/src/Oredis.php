 ...
 * $oo = new oredis( oo::$config['Redis'], false);
 * 
...
class Oredis{
	...
	public function __construct( $aServer, $persist=false, $aParam=array()){
		$this->aServer = $aServer;
		$this->persist = $persist;

		if(isset($aParam['seria'])){
			$this->seria = $aParam['seria'];
		}
	...
	public function set($key, $value, $Timeout=0){
		...
		for ($retry=0; $retry<2; $retry++){ //重试两次
			try {
				if($this->connect('set', $key)){
					$res = ($Timeout > 0) ? $this->oRedis->set( $key, $value, (int)$Timeout) : $this->oRedis->set( $key, $value);
				}
				break;
	...
	public function lLen($key){
		return $this->connect('lLen', $key) ? $this->oRedis->lLen($key) : false;
	}
	/**
	 * 取出队列的某一段.不存在则返回空数组
	 * @param String $key
    ...
    public function zAdd($key, $score, $value){
    	...
    	for ($retry=0; $retry<2; $retry++){ //重试两次
			try {
				$res = $this->connect('zAdd', $key) && $this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_NONE ) ?  $this->oRedis->zAdd($key, $score, $value) : false;
				break;
			}catch (RedisException $e){
				$this->close(); //显式关闭,强制重连
		...
		$this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_PHP );
		return $res;
    ...
    public function zRange($key, $start, $end, $withscores=false){
		$this->connect('zRange', $key) && $this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_NONE ) && $result = $this->oRedis->zRange($key, $start, $end, $withscores);
		$this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_PHP );
    ...
    public function zRevRange($key, $start, $end, $withscores=false){
    	for ($retry=0; $retry<2; $retry++){ //重试两次
			try {
				$res = $this->connect('zRevRange', $key) && $this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_NONE ) && is_array($result = $this->oRedis->zRevRange($key, $start, $end, $withscores)) ? $result : array();
				break;
			}catch (RedisException $e){
				$this->close(); //显式关闭,强制重连
    ...
    public function zRangeByScore($key, $start, $end, $options=array()){
		$this->connect('zRangeByScore', $key) && $this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_NONE ) && $result = $this->oRedis->zRangeByScore($key, $start, $end, $options);
		$this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_PHP );
		return is_array($result) ? $result : array();
    }
    /**
    ...
    public function zRevRangeByScore($key, $start, $end, $options=array()){
		$this->connect('zRevRangeByScore', $key) && $this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_NONE ) && $result = $this->oRedis->zRevRangeByScore($key, $start, $end, $options);
		$this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_PHP );
		return is_array($result) ? $result : array();
	}
	/**
    ...
    public function zCount($key, $start, $end){
		$this->connect('zCount', $key) && $this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_NONE ) && $result = $this->oRedis->zCount($key, $start, $end);
		$this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_PHP );
		return (int)$result;
    }
    /**
    ...
    public function zDeleteRangeByScore($key, $start, $end){
		$this->connect('zDeleteRangeByScore', $key) && $this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_NONE ) && $result = $this->oRedis->zDeleteRangeByScore($key, $start, $end);
		$this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_PHP );
		return (int)$result;
    }
    /**
    ...
    public function zDeleteRangeByRank($key, $start, $end){
		$this->connect('zDeleteRangeByRank', $key) && $this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_NONE ) && $result = $this->oRedis->zDeleteRangeByRank($key, $start, $end);
		$this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_PHP );
		return (int)$result;
	}
	/**
    ...
    public function zSize( $key){
		...
		for ($retry=0; $retry<2; $retry++){ //重试两次
			try {
				$res = $this->connect('zSize', $key) && $this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_NONE ) ? $this->oRedis->zSize($key) : 0;
				$this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_PHP );
				break;
			}catch (RedisException $e){
				$this->close(); //显式关闭,强制重连
    ...
    public function zScore($key, $member){
		...
		for ($retry=0; $retry<2; $retry++){ //重试两次
			try {
				$res = $this->connect('zScore', $key) && $this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_NONE ) ? $this->oRedis->zScore( $key, $member) : 0;
				$this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_PHP );
				break;
			}catch (RedisException $e){
				$this->close(); //显式关闭,强制重连
    ...
    public function zRank($key, $member){
		$result = $this->connect('zRank', $key) && $this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_NONE ) ? $this->oRedis->zRank( $key, $member) : 0;
		$this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_PHP );
		return $result;
    }
    /**
    ...
    public function zRevRank($key, $member){
		$result = $this->connect('zRevRank', $key) && $this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_NONE ) ? $this->oRedis->zRevRank( $key, $member) : 0;
		$this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_PHP );
		return $result;
    }
    /**
    ...
    public function zIncrBy($key, $value, $member){
    	...
    	for ($retry=0; $retry<2; $retry++){ //重试两次
			try {
				$res = $this->connect('zIncrBy', $key) && $this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_NONE ) ? $this->oRedis->zIncrBy( $key, $value, $member) : 0;
				$this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_PHP );
				break;
			}catch (RedisException $e){
				$this->close(); //显式关闭,强制重连
    ...
    public function zUnion($keyOutput, $arrayZSetKeys, $arrayWeights, $aggregateFunction){
		$result = $this->connect('zUnion', $keyOutput) && $this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_NONE ) ? $this->oRedis->zUnion( $keyOutput, $arrayZSetKeys, $arrayWeights, $aggregateFunction) : 0;
		$this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_PHP );
		return $result;
    }
    /**
    ...
    public function zInter($keyOutput, $arrayZSetKeys, $arrayWeights, $aggregateFunction){
		$result = $this->connect('zInter', $keyOutput) && $this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_NONE ) ? $this->oRedis->zInter( $keyOutput, $arrayZSetKeys, $arrayWeights, $aggregateFunction) : 0;
		$this->oRedis->setOption( Redis::OPT_SERIALIZER, Redis::SERIALIZER_PHP );
		return $result;
    }
	...
	private function errorlog($keys, $code, $msg, $die=false, $file = 'redisTimeout'){
